...
const MetricCards = () => {
  ...
  const { totalStreams, revenue, topArtist } = useMemo(() => {
    const totalStreams = topSongs.reduce((sum, day) => 
      sum + day.songs.reduce((songSum, song) => songSum + song.streams, 0), 0);

    const revenue = data.revenueDistribution.reduce((sum, item) => sum + item.value, 0);

    const artistStreamCounts = {};
    topSongs.forEach(day => {
      day.songs.forEach(song => {
        artistStreamCounts[song.artist] = (artistStreamCounts[song.artist] || 0) + song.streams;
      });
    });

    const topArtist = Object.entries(artistStreamCounts).reduce((top, [artist, streams]) => 
      streams > top.streams ? { name: artist, streams } : top, { name: '', streams: 0 });

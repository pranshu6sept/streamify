    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
        <div className="bg-white p-4 rounded-lg shadow">
          <UserGrowthChart
            key={`user-growth-${userGrowthKey}`}
            onError={resetCharts}
          />
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <RevenueDistributionChart
            key={`revenue-distribution-${pieChartsKey}`}
            onError={resetCharts}
          />
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
        <div className="bg-white p-4 rounded-lg shadow">
          <GenrePopularityChart
            key={`genre-popularity-${pieChartsKey}`}
            onError={resetCharts}
          />
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <UserDemographicsChart
            key={`user-demographics-${pieChartsKey}`}
            onError={resetCharts}
          />
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <DeviceUsageChart
            key={`device-usage-${pieChartsKey}`}
            onError={resetCharts}
          />
        </div>
      </div>
